...
list< pulliulli > get_suffix_tuple_list(string filename){
    // cout<<"get_suffix_tuple_list: "<<filename<<" : ";
    ifstream inFile;
...
int main(int argc, char** argv){
    ...
    // cout<<binned_kmer_dir<<" "<<prefix_idx_str<<" "<<min_presence_count<<" "<<filtered_kmer_dir<<"\n";

    for(unsigned long assembly_idx=0; assembly_idx<assembly_count; assembly_idx++){
        suffix_tuple_list.clear();

        filename = binned_kmer_dir + to_string(assembly_idx) + "_" + prefix_idx_str + "_mer";
        // cout<<"\t"<<filename<<"\t";
        suffix_tuple_list = get_suffix_tuple_list(filename);

        if(suffix_tuple_list.size() > 0)
            list_queue.push(suffix_tuple_list);

        filename = binned_kmer_dir + to_string(assembly_idx) + "_" + prefix_idx_str + "_mer_repeat";
        // cout<<"\t"<<filename<<"\n";
        suffix_tuple_list = get_suffix_tuple_list(filename);
    ...
    cout<<"sampled_kmer_filtering.cpp completed: "<<prefix_idx_str<<"\n";
